...
contract HeavenlyGuitars is ERC721Upgradeable, EIP712Upgradeable, AccessControlUpgradeable {
  ...
  event BaseURISet(string indexed previousBaseURI, string indexed newBaseURI); 
  ...
  function initialize(string memory name_, string memory symbol_, string calldata baseURI_) external initializer {
   ...
   function setBaseURI(string calldata baseURI_) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    baseURI = baseURI_;
    emit BaseURISet(previousBaseURI, baseURI_); 
  }

  function mintBatch(address[] calldata tos_, uint256[] calldata tokenIds_) external onlyRole(MINTER_ROLE) {
    if (tos_.length != tokenIds_.length) revert InvalidLength();
  ...
  function burnBatch(uint256[] calldata tokenIds_) external onlyRole(MINTER_ROLE) {

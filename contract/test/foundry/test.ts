...
describe("Token contract Guitar", function () {
  let contractAddress = "";
  let proxy: any = ""
  const paramDeployment: string[] = ["HeavenlyGuitars", "GCT", ""];
  const version: string = "1";
  const deadline: number = 9999999999;
  const tokenId: number = 1;
  const secondTokenId: number = 2;

  console.log("Start deploying HeavenlyGuitars...");
  console.log('.......................................');
  before(async function () {
    const HeavenlyGuitars = await ethers.getContractFactory("HeavenlyGuitars");
    proxy = await upgrades.deployProxy(HeavenlyGuitars, paramDeployment);
